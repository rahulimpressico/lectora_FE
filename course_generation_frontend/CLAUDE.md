...
- `editor/api.ts` — `getCourseContent`, `performAIOperation`, `saveSectionContent`, `downloadCourseArtifact` (`POST /jobs/{jobId}/artifacts/render-docx` with full snapshot), `saveToAzure` (`POST /jobs/{jobId}/artifacts/save-to-azure` with `{ course, courseSlug? }`), `deleteSectionAPI` (`DELETE /jobs/{jobId}/sections/{sectionId}`), `persistSectionOrder` (`PATCH /jobs/{jobId}/sections/reorder`), `updateCourseTitleAPI` (`PATCH /jobs/{jobId}/course`).
