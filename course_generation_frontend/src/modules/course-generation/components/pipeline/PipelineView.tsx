...
export function PipelineView({ jobId }: PipelineViewProps) {
  ...
  if (fatalError) {
    return (
      <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="relative z-20 shrink-0 flex items-center gap-3 bg-white/80 backdrop-blur-sm border-b border-slate-200/70 px-5 py-2.5">
          <button
            type="button"
        ...
        </div>{/* end inner centered card wrapper */}
      </div>
    )
  }
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      {/* ── Persistent top-left back button ──────────────────────────── */}
      <div className="relative z-20 shrink-0 flex items-center gap-3 bg-white/80 backdrop-blur-sm border-b border-slate-200/70 px-5 py-2.5">
        <button
          type="button"
          onClick={() => setPhase('three-panel')}
      ...
      </div>{/* end inner pipeline wrapper */}
    </div>
  )
}
