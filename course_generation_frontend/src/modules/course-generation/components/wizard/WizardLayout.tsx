...
export const WizardLayout = ({ children }: { children: ReactNode }) => {
  ...
  const pageTransition = {
    x: { type: 'spring' as const, stiffness: 340, damping: 36, mass: 0.85 },
    opacity: { duration: 0.2, ease: 'easeOut' as const },
    scale: { duration: 0.28, ease: [0.25, 0.46, 0.45, 0.94] as [number, number, number, number] },
  }

  const handleBack = () => {
    if (navConfig.backPhase) setPhase(navConfig.backPhase)
  }
                      ...
                      <div className="flex flex-col items-center gap-2 min-w-0">
                        {isCompleted ? (
                          <button
                            ...
                            className="w-9 h-9 rounded-full bg-indigo-600 flex items-center justify-center text-white shadow-sm shadow-indigo-200/60 hover:bg-indigo-700 hover:scale-105 active:scale-95 transition-all cursor-pointer"
                          >
          ...
          <div className="sm:hidden px-4 py-3.5">
            ...
            <div className="w-full bg-slate-100 rounded-full h-1 overflow-hidden">
              <div
                ...
                style={{ width: `${((safeIndex + 1) / WIZARD_STEPS.length) * 100}%` }}
              />
            </div>
          </div>
        </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  {navConfig.backPhase ? (
                    <button
                      type="button"
                      onClick={handleBack}
                      className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium text-slate-500 rounded-xl hover:bg-slate-100 hover:text-slate-700 transition-all duration-150 active:scale-95"
                    >
                  ...
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-2 px-6 py-2.5 text-sm font-semibold rounded-xl transition-all duration-150 active:scale-95',
                      'bg-indigo-600 text-white shadow-sm shadow-indigo-200/60',
                      'hover:bg-indigo-700 hover:shadow-md hover:shadow-indigo-200/60 hover:-translate-y-0.5',
                      'disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-sm disabled:hover:bg-indigo-600',
                    )}
                  >
