...
function StageDot({ stage }: { stage: PipelineStageState }) {
  ...
  if (stage.status === 'completed') {
    return (
      <div
        className={cn(base, 'w-7 h-7 bg-emerald-500 shadow-sm')}
...
// ── Inline blocker card (shown below pill row when S1/S2 retries) ─────────────
...
const STAGE_MESSAGES: Record<string, string[]> = {
  A1: [
    'Interpreting your study guide…',
    'Analysing document structure and sections…',
    'Extracting learning objectives and key concepts…',
  ],
  S1: [
    'Validating course structure and quality…',
    'Running content compliance checks…',
    'Ensuring all sections meet the required standards…',
  ],
  A2: [
    'Writing course content for each section…',
    'Crafting engaging learning material…',
    'Generating detailed explanations and examples…',
  ],
  S2: [
    'Reviewing generated content for accuracy…',
    'Checking word count targets and coverage…',
    'Validating knowledge check placements…',
  ],
  FINALIZATION: [
    'Finalising course structure…',
    'Assembling all sections together…',
    'Preparing your course for export…',
  ],
  EXPORT: [
    'Exporting to DOCX format…',
    'Packaging your course document…',
...
function useRotatingMessage(activeStageId: string | null): string {
  const msgs = activeStageId
    ...
    : ['Preparing pipeline…']
  const idx = Math.floor(Date.now() / 3_000) % msgs.length
  return msgs[idx]
}
...
export function PipelineView({ jobId }: PipelineViewProps) {
  ...
  // Stages that currently have blockers to surface inline
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50">
      <div className="relative flex-1 flex flex-col items-center justify-between overflow-y-auto py-10 px-4">
          ...
          {isProcessing && (
            <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-widest bg-indigo-50 border border-indigo-200 text-indigo-600">
              <span
                ...
                style={{ animation: 'pulse-ring 1.6s ease-out infinite', position: 'relative' }}
              />
        ...
        <div className="flex flex-col items-center gap-6 my-4">
          ...
          {isProcessing && (
            <p className="shimmer-text text-sm font-medium text-center max-w-xs" key={statusMessage}>
              {statusMessage}
            </p>
          )}
          {isCompleted && (
        ...
        <div className="flex flex-col items-center gap-5 w-full max-w-lg">
          ...
          {/* ── Inline validation blocker cards ───────────────────────────── */}
